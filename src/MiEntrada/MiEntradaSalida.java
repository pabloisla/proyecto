package MiEntrada;
	...
	public class MiEntradaSalida {
		...
		public static int solicitarEnteroPositivo(String mensaje) {
			// Variable que almacenara el entero introducido por teclado.
			int integer = 0;
			// Variable que almacenara un booleano que indicar si se le debe volver a pedir el dato al usuario.
			boolean flag = true;
		...
		public static char solicitarCaracter(String mensaje) {
			...
			while(flag) {
				// Pedimos el entero por pantalla.
				System.out.println(mensaje);
				// Comprobamos si el usuario esta introduciendo algo correcto usando la excepci�n del m�todo parseInt.
				try {
		...
		public static String solicitarCadena(String mensaje) {
			...
			while(flag) {
				...
				if (cadena.length() > 0) {
					// Si llegamos hasta aqui, es porque el usuario ha introducido un dato correcto y no se ha lanzado ninguna excepci�n.
					flag = false;
				}			
			}
